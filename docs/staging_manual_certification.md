...
2. **Aplicar dependencias.** Ejecutar `/opt/autonogrow/.venv/bin/python -m pip install -r
   backend/requirements.txt` y después `python -m pip check`. Esperado: instalación y `pip check`
   con exit 0. Si falla: devolver paquete/conflicto y últimas líneas sin URLs privadas.
3. **Verificar Alembic.** Ejecutar `alembic current`, `alembic heads` y, con backup confirmado,
   `python scripts/manage_migrations.py upgrade` seguido de `... validate`. Esperado: una head y
   current `20260814_19`. Si falla: devolver revisiones current/head y recomendación del script.
...
7. **Reiniciar workers.** Ejecutar `sudo systemctl restart autonogrow-worker.service
   autonogrow-instagram-publisher.service` y `python -m app.workers.instagram_publish_worker
   --check`. Esperado: ambas unidades activas y JSON `ok=true` sin jobs reclamados. Si falla:
   devolver unidad, ActiveState/NRestarts y salida segura de `--check`.
8. **Comprobar servicios.** Ejecutar `systemctl is-active/is-enabled` para backend, ambos workers y
   ...
   restart loop y un único scheduler de mantenimiento. Si falla: devolver unidad y propiedades.
...
13. **Ejecutar publication preflight.** Crear/aprobar un JPEG de prueba y ejecutar
    `python scripts/instagram_publication_preflight.py --business-id <ID> --content-id <ID> --json`.
