...
Ejecutar en orden. Para la campaña 10C no realizar los pasos provider-real históricos: el
publisher permanece stopped/disabled y Meta/WhatsApp no reciben llamadas. Ante un
...
1. **Desplegar el commit 10C.** Instalar la release mediante el mecanismo versionado del VPS en
...
3. **Verificar Alembic.** Ejecutar con `/opt/autonogrow/backend/.venv-next/bin/python` los comandos
   ...
   `20260822_23`. Si falla: devolver revisiones current/head y recomendación del script.
...
7. **Comprobar workers sin activarlos.** Reiniciar solo `autonogrow-worker.service` si ya estaba
   autorizado/activo. Confirmar `INSTAGRAM_PUBLISHING_WORKER_ENABLED=false` y que
   `autonogrow-instagram-publisher.service` sigue stopped/disabled. No ejecutar `--once` ni un
   preflight que requiera provider. Si falla: devolver unidad y ActiveState/NRestarts.
8. **Comprobar servicios.** Ejecutar `systemctl is-active/is-enabled` para backend, channel worker,
   publisher y `autonogrow-maintenance.timer`; consultar `systemctl list-timers`. Esperado: estado
   coincidente con la autorización, publisher stopped/disabled, sin restart loop y un único
   scheduler de mantenimiento a las 04:30 más hasta 10 minutos de jitter.
...
12. **Comprobar estado Meta sin provider call.** En Owner/Admin revisar únicamente el estado
    persistido y la capability Social. Esperado: disabled o action-required coherente; no reclamar
    jobs, renovar tokens ni publicar. La validación real queda para una operación separada y
    expresamente autorizada mediante `instagram_real_publish_runbook.md`.
13. **Validar capabilities/readiness.** Ejecutar `scripts/check_pilot_configuration.py --json`.
    Esperado: sin inconsistencias y con blockers/warnings del business QA revisados.
14. **Revisar logs.** Revisar la ventana con `journalctl` para los servicios autorizados y Caddy.
