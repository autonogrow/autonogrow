...
function ownerIntegrationRecommendation(record) {
  ...
  if (record.health?.subscription_status === "missing") return `Reparar conexión con ${record.control.channel === "instagram" ? "Instagram" : "WhatsApp"}`;
  if (["suspended", "revoked"].includes(record.control.status)) return "Revisar el control comercial";
  if (!ownerIntegrationIsActive(record)) return "Completar la conexión";
...
function ownerIntegrationHealthDiagnosis(record) {
  ...
  if (record.health?.subscription_status === "missing") return "La conexión necesaria para recibir actualizaciones no está configurada.";
...
function ownerIntegrationHealthActions(record) {
  ...
  if (record.health?.subscription_status === "missing") return '<button class="button button-secondary button-small" type="button" data-owner-integration-retry-subscription>Reparar conexión</button>';
...
async function retryOwnerIntegrationSubscription() {
  ...
  if (!record) return;
  const channelName = record.control.channel === "instagram" ? "Instagram" : "WhatsApp";
  const confirmed = await confirmOwnerCriticalAction({ title: `Reparar conexión con ${channelName}`, resource: `${channelName} · ${record.business.name}`, current: "Recepción de actualizaciones no configurada", next: "Reparación solicitada", consequence: `Volveremos a configurar la conexión necesaria para recibir correctamente las actualizaciones de ${channelName}.`, confirmLabel: "Reparar conexión", danger: false, requiresReason: false, action: () => ownerHubRequest(`/api/owner/businesses/${encodeURIComponent(record.business.id)}/channels/${encodeURIComponent(record.control.channel)}/retry-subscription`, { method: "POST" }, "No se pudo solicitar la reparación.") });
  if (confirmed) await refreshOwnerIntegrationContext();
}
