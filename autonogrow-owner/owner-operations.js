...
function renderOwnerMaintenance() {
  ...
  target.innerHTML = `<article class="owner-maintenance-state ${maintenance.enabled ? "active" : ""}"><span class="ag-badge ${maintenance.enabled ? "ag-badge--danger" : "ag-badge--success"}">${maintenance.enabled ? "Mantenimiento activo" : "Operación normal"}</span><p><strong>Estado actual</strong>${maintenance.enabled ? "El middleware aplica el alcance configurado por el backend." : "El modo mantenimiento está desactivado."}</p><p><strong>Motivo</strong>${escapeHtml(maintenance.reason || "Sin motivo registrado")}</p><p><strong>Último cambio</strong>${escapeHtml(ownerOperationalDate(maintenance.updated_at))}</p><p>Los datos se conservan. Este panel no ejecuta backups, restauraciones, despliegues ni consultas.</p></article>`;
