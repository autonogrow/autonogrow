...
class AdminPollingFrontendTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.admin_js = (root / "autonogrow-admin" / "admin.js").read_text(
            encoding="utf-8"
        )
        cls.admin_html = (root / "autonogrow-admin" / "index.html").read_text(
            encoding="utf-8"
        )
        cls.admin_css = (root / "autonogrow-admin" / "styles.css").read_text(
            encoding="utf-8"
        )
    ...
    def test_polling_is_non_destructive_and_skips_static_data(self):
        ...
        operations_block = self.admin_js.split(
            'adminPollingTasks.set("operations"', 1
        )[1].split('function updateAdminSyncIndicator', 1)[0]
